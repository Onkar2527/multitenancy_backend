...
async function generateToken(req) {
    ...
    const host = process.env.CBS_API_HOST || req.cbsApiHost || 'https://10.35.250.3';
    const port = process.env.CBS_API_PORT || req.cbsApiPort || 9854;
...
async function cacheMasters(req) {
    ...
    let [masters_data, masters_fields] = await pool.query(getmasterQ);
    ...
    for (let table of masters_data) { // console
        ...
        for (let result of masterResult) {
            ...
            let [checkR2, checkF2] = await pool.query(checkQ);
...
async function buildOnboardPayloadData(req, applicant_id) {
    const pool = req.db;
    if (!pool) {
    ...
    const account_opening_data = {
        ...
        "docdtl_obj": {
            "schemecode": Number(depositR?.SCHEME_CODE || 0),
            "docid": Number(personalR?.ID_PROOF || 0),
            "changeno": 1,
            "bankcode": 1,
            "brncode": await getBranchFromCBS(masterPool, basicR.CREATED_BRANCH_ID),
            "acctobeopn_atbrncd": await getBranchFromCBS(masterPool, basicR.CREATED_BRANCH_ID),
...
exports.getOnboardPayload = async (req, res) => {
    try {
        const applicant_id = req.body.APPLICANT_ID;
        const { account_opening_data } = await buildOnboardPayloadData(req, applicant_id);
        res.send({
            code: 200,
            message: 'success',
        ...
        console.error('❌ getOnboardPayload error:', error);
        res.status(400).send({
            code: 400,
...
exports.onBoardCustomer = async (req, res) => {
    try {
        ...
        const { account_opening_data, basicR, personalR, depositR } = await buildOnboardPayloadData(req, applicant_id);
